...
saveScheduleToFirebase(subjects);
}

async function saveScheduleToFirebase(subjects){
    try {
        const scheduleRef = doc(db, "schedules", currentDay);
...
async function loadSchedule(){
    try {
        ...
        const data = snapshot.exists() ? (snapshot.data().subjects || []) : [];
        ...
        console.error("時間割の読み込みエラー:", error);
        const inputs=document.querySelectorAll(".subject");
